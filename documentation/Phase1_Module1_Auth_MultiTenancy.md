...
- `prisma/migrations/manual/` (holding the Row-Level Security SQL from Phase 0 Module 1) had to be relocated to `prisma/manual-sql/` — Prisma's migrate engine scans every subfolder under `prisma/migrations/` expecting a real migration and errored on it.
- Installed PostgreSQL 17 natively (not Docker — avoided the heavier WSL2/virtualization requirement for local dev; Docker Compose remains the Phase 7 deployment target) and Node.js LTS.
