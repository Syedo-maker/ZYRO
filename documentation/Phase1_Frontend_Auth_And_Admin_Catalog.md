...
- Vite + React 18 + TypeScript scaffold, Tailwind v4 (CSS-first `@theme` config, not the old `tailwind.config.js` — v4 changed this), design tokens copied exactly from `ComponentInventory.dc.html` so the built app matches the approved wireframes rather than drifting.
