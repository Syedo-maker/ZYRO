...
- Errors are RFC 7807 problem details (`application/problem+json`, a shared `Error` schema with a stable `type` URI), not ad hoc `{error: string}` shapes — one error shape for both students to handle in the frontend, once.
