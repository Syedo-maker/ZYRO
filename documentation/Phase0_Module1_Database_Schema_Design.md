...
- **Two-layer isolation**: Prisma middleware (Phase 1, application layer) plus the RLS policies in `001_enable_rls.sql` (database layer). If the app-layer middleware has a bug, RLS is the backstop, and this is exactly the tenant-isolation testing target called out in Implementation_Plan.md Phase 6.
...
| AiGeneratedContent → history | 1:few, bounded (≤5) | **Embed, capped** | Cheap "compare with last generation" UX, not a compliance audit trail, so it doesn't need the full document-versioning pattern (separate revisions collection). |
